/**
 * @file <argos3/plugins/robots/foot-bot/control_interface/ci_efootbot_proximity_sensor.cpp>
 *
 * @author Carlo Pinciroli <ilpincy@gmail.com>
 */

#include "ci_efootbot_proximity_sensor.h"
...
namespace argos {
   ...
   CCI_EFootBotProximitySensor::CCI_EFootBotProximitySensor() :
      m_tReadings(24) {
   ...
   const CCI_EFootBotProximitySensor::TReadings& CCI_EFootBotProximitySensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_EFootBotProximitySensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_EFootBotProximitySensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_EFootBotProximitySensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_EFootBotProximitySensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
