/**
 * @file <argos3/plugins/robots/foot-bot/control_interface/ci_efootbot_light_sensor.cpp>
 *
 * @author Carlo Pinciroli - <ilpincy@gmail.com>
 */

#include "ci_efootbot_light_sensor.h"
...
namespace argos {
   ...
   CCI_EFootBotLightSensor::CCI_EFootBotLightSensor() :
      m_tReadings(24) {
   ...
   const CCI_EFootBotLightSensor::TReadings& CCI_EFootBotLightSensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_EFootBotLightSensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_EFootBotLightSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_EFootBotLightSensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_EFootBotLightSensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
