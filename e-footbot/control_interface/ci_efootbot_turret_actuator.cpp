/**
 * @file <argos3/plugins/robots/foot-bot/control_interface/ci_efootbot_turret_actuator.cpp>
 *
 * @author Carlo Pinciroli - <ilpincy@gmail.com>
 */

#include "ci_efootbot_turret_actuator.h"
...
namespace argos {
  ...
  const CRange<SInt32> CCI_EFootBotTurretActuator::SPEED_RANGE(-4,4);
  const CRange<Real>   CCI_EFootBotTurretActuator::NORMALIZED_SPEED_RANGE(-1.0,1.0);
  ...
  int LuaTurretSetRotation(lua_State* pt_lua_state) {
    ...
    /* Perform action */
    CLuaUtility::GetDeviceInstance<CCI_EFootBotTurretActuator>(pt_lua_state, "turret")->
  ...
  void CCI_EFootBotTurretActuator::SetActiveWithRotation(const CRadians& c_angle) {
    SetPositionControlMode();
    SetRotation(c_angle);
  }
  ...
  void CCI_EFootBotTurretActuator::SetSpeedControlMode() {
  ...
  void CCI_EFootBotTurretActuator::SetPositionControlMode() {
  ...
  void CCI_EFootBotTurretActuator::SetPassiveMode() {
...
#ifdef ARGOS_WITH_LUA
  void CCI_EFootBotTurretActuator::CreateLuaState(lua_State* pt_lua_state) {
