/**
 * @file <argos3/plugins/robots/foot-bot/simulator/dynamics2d_efootbot_model.cpp>
 *
 * @author Carlo Pinciroli - <ilpincy@gmail.com>
 */

#include "dynamics2d_efootbot_model.h"
...
namespace argos {
   ...
   static const Real EFOOTBOT_INTERWHEEL_DISTANCE      = 0.14f;
   static const Real EFOOTBOT_HEIGHT                   = 0.146899733f;

   static const Real EFOOTBOT_MAX_FORCE                = 15.f;
   static const Real EFOOTBOT_MAX_TORQUE               = 150.f;

   enum EFOOTBOT_WHEELS {
      EFOOTBOT_LEFT_WHEEL = 0,
      EFOOTBOT_RIGHT_WHEEL = 1
   };
   ...
   CDynamics2DEFootBotModel::CDynamics2DEFootBotModel(CDynamics2DEngine& c_engine,
                                                    CEFootBotEntity& c_entity) :
      CDynamics2DMultiBodyObjectModel(c_engine, c_entity),
      m_cEFootBotEntity(c_entity),
      m_cWheeledEntity(m_cEFootBotEntity.GetWheeledEntity()),
      m_cGripperEntity(c_entity.GetGripperEquippedEntity()),
      m_cDiffSteering(c_engine,
                      EFOOTBOT_MAX_FORCE,
                      EFOOTBOT_MAX_TORQUE,
                      EFOOTBOT_INTERWHEEL_DISTANCE),
      m_pcGripper(NULL),
      m_pcGrippable(NULL),
      m_fMass(1.6f),
      m_fCurrentWheelVelocity(m_cWheeledEntity.GetWheelVelocities()),
      m_unLastTurretMode(m_cEFootBotEntity.GetTurretEntity().GetMode()) {
      RegisterAnchorMethod<CDynamics2DEFootBotModel>(
         GetEmbodiedEntity().GetOriginAnchor(),
         &CDynamics2DEFootBotModel::UpdateOriginAnchor);
      RegisterAnchorMethod<CDynamics2DEFootBotModel>(
         GetEmbodiedEntity().GetAnchor("turret"),
         &CDynamics2DEFootBotModel::UpdateTurretAnchor);
      RegisterAnchorMethod<CDynamics2DEFootBotModel>(
         GetEmbodiedEntity().GetAnchor("perspective_camera"),
         &CDynamics2DEFootBotModel::UpdatePerspectiveCameraAnchor);
         ...
         cpSpaceAddBody(GetDynamics2DEngine().GetPhysicsSpace(),
                        cpBodyNew(m_fMass,
                                  cpMomentForCircle(m_fMass,
                                                    0.0f,
                                                    EFOOTBOT_RADIUS + EFOOTBOT_RADIUS,
                                                    cpvzero)));
      ...
      AddBody(m_ptActualBaseBody, cpvzero, 0, EFOOTBOT_HEIGHT);
      ...
      m_ptBaseGripperAngularMotion->maxForce = EFOOTBOT_MAX_TORQUE; /* limit the dragging torque */
      /* Add the gripper body so that the default methods work as expected */
      AddBody(m_ptActualGripperBody, cpvzero, 0, EFOOTBOT_HEIGHT);
   ...
   CDynamics2DEFootBotModel::~CDynamics2DEFootBotModel() {
      delete m_pcGripper;
   ...
   void CDynamics2DEFootBotModel::Reset() {
   ...
   void CDynamics2DEFootBotModel::CalculateBoundingBox() {
      ...
      GetBoundingBox().MaxCorner.SetZ(GetDynamics2DEngine().GetElevation() + EFOOTBOT_HEIGHT);
   }

   /****************************************/
   /****************************************/

   void CDynamics2DEFootBotModel::UpdateFromEntityStatus() {
      /* Do we want to move? */
      if((m_fCurrentWheelVelocity[EFOOTBOT_LEFT_WHEEL] != 0.0f) ||
         (m_fCurrentWheelVelocity[EFOOTBOT_RIGHT_WHEEL] != 0.0f)) {
         m_cDiffSteering.SetWheelVelocity(m_fCurrentWheelVelocity[EFOOTBOT_LEFT_WHEEL],
                                          m_fCurrentWheelVelocity[EFOOTBOT_RIGHT_WHEEL]);
      }
      else {
      ...
      if(m_cEFootBotEntity.GetTurretEntity().GetMode() != m_unLastTurretMode) {
         /* Enable or disable the anchor */
         if(m_cEFootBotEntity.GetTurretEntity().GetMode() != MODE_OFF) {
         ...
         switch(m_unLastTurretMode) {
            case MODE_OFF:
            case MODE_PASSIVE:
               switch(m_cEFootBotEntity.GetTurretEntity().GetMode()) {
                  case MODE_POSITION_CONTROL:
                  case MODE_SPEED_CONTROL:
      ...
      switch(m_unLastTurretMode) {
         ...
         case MODE_POSITION_CONTROL: {
            Real fCurRotErr = NormalizedDifference(
               m_cEFootBotEntity.GetTurretEntity().GetDesiredRotation(),
         ...
         case MODE_SPEED_CONTROL:
            m_ptControlGripperBody->w =
               m_cDiffSteering.GetAngularVelocity() +
               m_cEFootBotEntity.GetTurretEntity().GetDesiredRotationSpeed();
            break;
         case MODE_OFF:
         case MODE_PASSIVE:
   ...
   void CDynamics2DEFootBotModel::TurretPassiveToActive() {
      ...
      m_ptGripperControlAngularMotion->maxForce = EFOOTBOT_MAX_TORQUE; /* limit the dragging torque */
   }

   /****************************************/
   /****************************************/

   void CDynamics2DEFootBotModel::TurretActiveToPassive() {
   ...
   void CDynamics2DEFootBotModel::UpdateOriginAnchor(SAnchor& s_anchor) {
   ...
   void CDynamics2DEFootBotModel::UpdateTurretAnchor(SAnchor& s_anchor) {
   ...
   void CDynamics2DEFootBotModel::UpdatePerspectiveCameraAnchor(SAnchor& s_anchor) {
   ...
   REGISTER_STANDARD_DYNAMICS2D_OPERATIONS_ON_ENTITY(CEFootBotEntity, CDynamics2DEFootBotModel);
