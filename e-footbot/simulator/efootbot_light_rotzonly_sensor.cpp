/**
 * @file <argos3/plugins/robots/foot-bot/simulator/efootbot_light_rotzonly_sensor.cpp>
 *
...
#include "efootbot_light_rotzonly_sensor.h"
...
namespace argos {
   ...
   CEFootBotLightRotZOnlySensor::CEFootBotLightRotZOnlySensor() :
   ...
   void CEFootBotLightRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
      try {
   ...
   void CEFootBotLightRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         /* Show rays? */
   ...
   void CEFootBotLightRotZOnlySensor::Update() {
      /* Erase readings */
      ...
      for(CSpace::TMapPerType::iterator it = mapLights.begin();
         ...
         if(cLight.GetIntensity() > 0.0f) {
            ...
            if(! GetClosestEmbodiedEntityIntersectedByRay(sIntersection,
               ...
               /*
                * Find closest sensor index to point at which ray hits efootbot body
                * Rotate whole body by half a sensor spacing (corresponding to placement of first sensor)
                * Division says how many sensor spacings there are between first sensor and point at which ray hits efootbot body
   ...
   void CEFootBotLightRotZOnlySensor::Reset() {
   ...
   REGISTER_SENSOR(CEFootBotLightRotZOnlySensor,
                   "efootbot_light", "rot_z_only",
                   ...
                   "      <sensors>\n"
                   "        ...\n"
                   "        <efootbot_light implementation=\"rot_z_only\" />\n"
                   "        ...\n"
                   "      </sensors>\n"
                   "      ...\n"
                   "    </my_controller>\n"
